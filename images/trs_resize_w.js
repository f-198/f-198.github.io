...
function showApp(){
    ...
    if ($("#iframe_header").length > 0) {
      $("#iframe_header_p",parent.document).height(Math.ceil($("#iframe_header").outerHeight()*winWidth/750));
    }
    if ($("#iframe_footer").length > 0) {
      $("#iframe_footer_p", parent.document).height(Math.ceil($("#iframe_footer").outerHeight() * winWidth / 750));
    }
    }
	else{
		$("html").css({"zoom":"","transform":""});
    if ($("#iframe_header").length > 0) {
      $("#iframe_header_p", parent.document).height(Math.ceil($("#iframe_header").outerHeight()));
    }
    if ($("#iframe_footer").length > 0) {
      $("#iframe_footer_p", parent.document).height(Math.ceil($("#iframe_footer").outerHeight()));
    }
	}
}
