...
var $dp, WdatePicker; (function() {
	...
	A = $.$dpPath || J();
	if ($.$wdate) K(A + "./trs_search_WdatePicker.css");
	V = Y;
	if ($.$crossFrame) {
		try {
	...
	function I(G, _) {
		...
		function E(K, J) {
			var I = V[N].domain,
			F = false,
			G = "<iframe hideFocus=true width=9 height=7 frameborder=0 border=0 style=\"width:400px;height:400px;\" scrolling=no src=\"about:blank\"></iframe>";
			K.innerHTML = G;
			var _ = $.$langList,
			D = $.$skinList,
			H;
			...
			function C() {
				...
				if(trs.IsPC()){
					...
					opt.zoom = 2,
					opt.style = ".WdateDiv{width:400px;height:400px;}.WdateDiv>div{width:198px;}"
				}
				...
				for (var C = 0; C < D.length; C++) if (D[C].name == J.skin) $.push("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + A + "./trs_search_datepicker.css\" charset=\"" + D[C].charset + "\"/><style>"+ opt.style +"</style>");
				$.push("<script src=\"" + A + "trs_search_calendar.js\"></script>");
				$.push("</head><body leftmargin=\"0\" topmargin=\"0\" tabindex=0><div style=\"display:none\"><script type=\"text/javascript\">document.write(unescape(\"%3Cscript src='//stat.www.gov.cn/webdig.js?z=1' type='text/javascript'%3E%3C/script%3E\"));</script><script type=\"text/javascript\">wd_paramtracker(\"_wdxid=000000000000000000000000000000000000000000\")</script></div><\/body><\/html>");
				...
				J.setPos = B;
				J.onload = Z;
				H.write("<html style='zoom:"+ opt.zoom +"'>");
				H.cfg = J;
				H.write($.join(""));
				H.close()
			}
