...
(function() {
  ...
  if (document.location.hostname === "www.w3.org" && /^\/TR\/\d{4}\//.test(document.location.pathname)) {
    ...
    request.open('GET', '../../TR/tr-outdated-spec/index.html');
    ...
    request.onerror = function() {
      console.error("Request to ../../TR/tr-outdated-spec/index.html failed.");
    };
  ...
  /* Dark mode toggle */
  const darkCss = document.querySelector('link[rel~="stylesheet"][href^="../../../assets/www.w3.org/912adcb372dd_dark.bin"]');
  if (darkCss) {
    ...
    if (!document.getElementById(tocThemeToggle)) {
      render.innerHTML = `
        <a id="toc-theme-toggle" role="radiogroup" aria-label="Select a color scheme">
          <span aria-hidden="true"><img src="../../../assets/www.w3.org/771c68a16a4c_dark.svg" title="theme toggle icon" /></span>
          <span>
